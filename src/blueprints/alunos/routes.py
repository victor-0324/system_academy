...
def mostrar_detalhes(aluno_id):
            ...
            medidas.append(
                {
                    ...
                    "data_atualizacao": (
                        medida.data_atualizacao.strftime("%d/%m/%Y")
                        if isinstance(medida.data_atualizacao, datetime)
                        else medida.data_atualizacao
                    ),
                }
            )
    ...
    return render_template(
        "pages/alunos/detalhes/index.jinja",
        ...
        data_pagamento_atual=data_pagamento_atual,
        manifest=manifest,
        medidas=medidas,
    )
...
def atualizar_ex(aluno_id):
    ...
    if request.method == "POST":
        try:
            ...
            sucesso = querys_instance.atualizar_exercicios_aluno(
                nome_aluno, filtro_dias, categoria_id
            )
        ...
        for exercicio in exercicios:
            if exercicio.atualizacao:
                ...
                exercicio.atualizacao_formatada = exercicio.atualizacao.strftime(
                    "%d/%m/%Y"
                )
        ...
        return render_template(
            "pages/alunos/exercicios/index.jinja",
            manifest=manifest,
                ...
                jsonify(
                    {
                        ...
                        "historico_depois": historico_depois,
                    }
                ),
                200,
            )
...
def cadastrar_ex():
    if request.method == "POST":
        ...
        querys_instance.cadastrar_ex(exercicio, serie, repeticao, descanso, carga)
...
def busca_adicionar():
    ...
    if request.method == "POST":
        try:
            ...
            exercicios = querys_instance.buscar_exercicios_por_nome(
                aluno_name, filtro_dias
            )
            ...
            sucesso_atualizacao = querys_instance.atualizar_exercicios_filtry(
                aluno_id, exercicios
            )
...
def editar_exercicio(exercicio_id):
    ...
    try:
        ...
        # que permitirá ao usuário fornecer os novos dados.
            ...
            if exercicio:
                return render_template(
                    "formulario_edicao_exercicio.html", exercicio=exercicio
                )
            else:
...
clientes_app.add_url_rule(
    "/exercicios/rest/<int:_id>", view_func=exercicio_aluno_view, methods=["DELETE"]
)
